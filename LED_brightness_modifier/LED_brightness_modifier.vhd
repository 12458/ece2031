...
type gamma_array_t is array (0 to 255) of unsigned(7 downto 0);
...
constant gamma8 : gamma_array_t := (
 ...
 252 => x"EE",253 => x"F1",254 => x"F4",255 => x"FF"
  ...
  signal cnt_8bit : unsigned(7 downto 0) := x"00";  
  signal gamma_val: unsigned(7 downto 0);    
  signal brightness: unsigned(7 downto 0);  
begin

  brightness <= unsigned(brightnessIn);
  process(clk)
  begin
    if rising_edge(clk) then
      if reset = '1' then
        ...
        gamma_val <= gamma8(to_integer(brightness));
      end if;
  ...
  output <= '1' when cnt_8bit < gamma_val else '0';
